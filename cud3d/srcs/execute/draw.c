...
/*   Updated: 2024/02/22 15:27:51 by tomuller         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// can make the image into a struct, so it doesn't need to be created each frame
{
	...
	img.img = mlx_new_image(lst->mlx->mlx, X_RES, Y_RES / 2);
	...
	mlx_put_image_to_window(lst->mlx->mlx, lst->mlx->win, img.img, 0, y_start);
}
...
void	draw_mmplayer(t_map *lst)
{
	...
	x = lst->px * 10;
	y = lst->py * 10;
	mlx_put_image_to_window(lst->mlx->mlx, lst->mlx->win,
